...
class ReflectionAnalytics:
    ...
    def reflections_by_branch(self, project: Optional[str] = None) -> Dict[str, int]:
        """
    ...
    def synergy_trend(
        ...
        for date_key, scores in sorted(by_date.items()):
            trend.append({
                "date": date_key,
                "average_synergy": statistics.mean(scores),
                "count": len(scores)
            })
...
class QueryBuilder:
    ...
    def filter_synergy_min(self, min_score: int) -> 'QueryBuilder':
        ...
        self.filters.append(
            lambda r: r.get("reflections", {}).get("ai_synergy", 0) >= min_score
        )
        return self
    ...
    def filter_confidence_min(self, min_score: int) -> 'QueryBuilder':
        ...
        self.filters.append(
            lambda r: r.get("reflections", {}).get("confidence", 0) >= min_score
        )
        return self
